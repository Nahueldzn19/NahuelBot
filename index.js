...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Nahuel❤\n' // Nombre
            + 'ORG:Nahuel;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=595992489510:+595 992 489510\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa Nahuel,
...
if (budy.includes("https://wa.me/")){
		...
		reply(`*LINK DE WHATSAPP DETECTADO 📢* ${sender.split("@")[0]} hmno, valiste mrd cagon`)
		setTimeout( () => {
...
if (budy.includes("https://www.facebook.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE FACEBOOK DETECTADO 📢* ${sender.split("@")[0]} hmno, valiste mrd cagon`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
...
//FUNCION ANTILINK
	...
	if (budy.includes("https://discord.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE DISCORD DETECTADO 📢* ${sender.split("@")[0]} hmno, valiste mrd cagon`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
	...
	if (budy.includes("https://s.kwai.app/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE KWAI DETECTADO 📢* ${sender.split("@")[0]} hmno, valiste mrd cagon`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
	...
	if (budy.includes("https://www.instagram.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE INSTAGRAM DETECTADO 📢* ${sender.split("@")[0]} hmno, valiste mrd cagon`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
	...
	if (budy.includes("https://vm.tiktok.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE TIK TOK DETECTADO 📢* ${sender.split("@")[0]} hmno, valiste mrd cagon`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'todos':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 *TODOS* 〙✪══\n╠➥'+teks+'╚═〘 _*by nahuel*_ 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
